...
from preprocessing.util import Time

TISSUE_HIGH_THRESH = 80
TISSUE_MED_THRESH = 50
...
def generate_top_tile_summaries(tile_sum, np_img, display=True, save_summary=False, show_top_stats=True,
  ...
  top_tiles = tile_sum.top_tiles(pick_random_tiles=True)
  for t in top_tiles:
    border_color = tile_border_color(t.tissue_percentage)
    tile_border(draw, t.r_s + z, t.r_e + z, t.c_s, t.c_e, border_color)
    tile_border(draw_orig, t.r_s + z, t.r_e + z, t.c_s, t.c_e, border_color)
  ...
  if show_top_stats:
    add_tile_stats_to_top_tile_summary(summary, top_tiles, z)
    add_tile_stats_to_top_tile_summary(summary_orig, top_tiles, z)
...
def tile_border(draw, r_s, r_e, c_s, c_e, color, border_size=TILE_BORDER_SIZE):
  ...
  for x in range(0, border_size):
    x0 = np.minimum(c_s + x, c_e - 1 - x)
    x1 = np.maximum(c_s + x, c_e - 1 - x)
    y0 = np.minimum(r_s + x, r_e - 1 - x)
    y1 = np.maximum(r_s + x, r_e - 1 - x)
...
def summary_and_tiles(slide_num, display=True, save_summary=False, save_data=True, save_top_tiles=True, mask=None, datatable=None):
  """
  ...
  if save_top_tiles:
    for tile in tile_sum.top_tiles(pick_random_tiles=False):
      tile.save_tile(mask=mask, datatable=datatable)
  return tile_sum
...
def tile_to_pil_tile(tile, mask=None, datatable=None):
  """
  ...
  t = tile
  global CUR_SLIDE
  if datatable is None: slide_filepath = slide.get_training_slide_path(t.slide_num)
  else: slide_filepath = datatable.iloc[t.slide_num].filepath

  if CUR_SLIDE is None: CUR_SLIDE = slide.open_slide(slide_filepath)

  x, y = t.o_c_s, t.o_r_s
  w, h = t.o_c_e - t.o_c_s, t.o_r_e - t.o_r_s
  tile_region = CUR_SLIDE.read_region((x, y), 0, (w, h))
  pil_img = tile_region.convert("RGB")
  if mask:
    pil_img = PIL.Image.composite(pil_img, PIL.Image.fromarray(np.ones(shape=(w, h))*255).convert('RGB'), mask.crop((x, y, x+w, y+h)))

  # RGBA to RGB

  if CUR_SLIDE.properties['aperio.AppMag']=='20':
    pil_img = pil_img.resize(tuple([dim*2 for dim in pil_img.size]), PIL.Image.BILINEAR)
  return pil_img
...
def save_display_tile(tile, mask=None, save=True, display=False, datatable=None):
  """
  ...
  """
  tile_pil_img = tile_to_pil_tile(tile, mask, datatable)
...
def image_list_to_tiles(image_num_list, display=False, save_summary=True, save_data=True, save_top_tiles=True, mask=None, datatable=None):
  """
  ...
  for slide_num in image_num_list:
    tile_summary = summary_and_tiles(slide_num, display, save_summary, save_data, save_top_tiles, mask, datatable)
    tile_summaries_dict[slide_num] = tile_summary
  return image_num_list, tile_summaries_dict


def image_range_to_tiles(start_ind, end_ind, display=False, save_summary=True, save_data=True, save_top_tiles=True, mask=None, datatable=None):
  """
...
def singleprocess_filtered_images_to_tiles(display=False, save_summary=True, save_data=True, save_top_tiles=True,
                                           html=True, image_num_list=None, mask=None, datatable=None):
  """
...
def image_row(slide_num, tile_summary, data_link):
  ...
  top_tiles = tile_summary.top_tiles(pick_random_tiles=True)
...
def display_image_with_rgb_histograms(np_rgb, scale_up=False):
  """
...
class TileSummary:
  ...
  def top_tiles(self, pick_random_tiles=False):
    """
    ...
    """
    if not pick_random_tiles:
      sorted_tiles = self.tiles_by_score()
    else:
      if self.random_tiles != []: return self.random_tiles
      candidate_tiles = [tile for tile in self.tiles if tile.tissue_percentage > 0]
      sorted_tiles = random.sample(candidate_tiles, len(candidate_tiles))
    if NUM_TOP_TILES is not None: top_tiles = self.random_tiles = sorted_tiles[:NUM_TOP_TILES]
    else: top_tiles = self.random_tiles = sorted_tiles
    return top_tiles
...
class Tile:
  ...
  def save_tile(self, mask=None, datatable=None):
    save_display_tile(self, mask, save=True, display=False, datatable=datatable)
