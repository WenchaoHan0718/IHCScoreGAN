...
def visualize_instances_dict(
    ...
    for idx, [inst_id, inst_info] in enumerate(inst_dict.items()):
        ...
        # cv2.drawContours(overlay, [inst_contour], -1, inst_colour, thickness=line_thickness) # contour
        # cv2.drawContours(overlay, [inst_contour], -1, [0,0,255], thickness=line_thickness) # blue border

        # random_shade = np.random.rand()
        # if inst_colour == [0, 0, 0]:
        #     inst_colour = list(np.array([0, 255, 0])*random_shade) if np.random.rand() > ratio else list(np.array([255, 0, 0])*random_shade)
        # else: inst_colour = list(np.array(inst_colour)*random_shade)
        # cv2.drawContours(overlay, [inst_contour], -1, inst_colour, thickness=cv2.FILLED)
