...
from segmentation.misc.wsi_handler import get_file_handler

from . import base

import preprocessing.mask as mask
...
class InferManager(base.InferManager):
    ...
    def process_single_file(self, wsi_path, msk_path, output_dir, counts=None):
        ...
        if msk_path is not None and os.path.isfile(msk_path):
            log_info(
                f"Preparing mask from {msk_path}"
            )
            if '.xml' in msk_path:
                self.wsi_mask = np.array(mask.xml_to_mask(msk_path, 1, tuple(np.flip(self.wsi_proc_shape)), counts))
            else:
    ...
    def process_wsi_list(self, run_args, counts=None):
        ...
        if '.svs' in self.input_dir: 
            wsi_path_list = [self.input_dir]
        else: wsi_path_list = glob.glob(self.input_dir + "/*")
        ...
        for wsi_path in wsi_path_list[:]:
            ...
            if '.xml' in self.input_mask_dir: msk_path = self.input_mask_dir
            else: msk_path = [os.path.join(self.input_mask_dir, file) for file in os.listdir(self.input_mask_dir) if wsi_base_name in file][0]
            ...
            try:
                ...
                self.process_single_file(wsi_path, msk_path, self.output_dir, counts=counts)
                log_info("Finish")
            except Exception:
