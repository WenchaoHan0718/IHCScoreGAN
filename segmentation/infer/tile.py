...
import tqdm
from segmentation.dataloader.infer_loader import SerializeArray, SerializeFileList
from segmentation.misc.utils import (
    color_deconvolution,
    cropping_center,
    get_bounding_box,
    log_debug,
...
from segmentation.misc.viz_utils import colorize, visualize_instances_dict
from skimage import color

from segmentation import convert_format
from . import base
...
class InferManager(base.InferManager):
    ...
    def process_file_list(self, run_args):
        ...
        if all([os.path.isdir(file) for file in file_path_list]):
            file_path_list = []
            for file in [[os.path.join(root, file) for file in file_list] for root, _, file_list in os.walk(self.input_dir)]: file_path_list += file
        ...
        os.makedirs(self.output_dir + '/mat/'    , exist_ok=True)
        os.makedirs(self.output_dir + '/overlay/', exist_ok=True)
        if self.save_qupath:
            os.makedirs(self.output_dir + "/qupath/", exist_ok=True)

        processed_files = [os.path.join(self.input_dir, file.split('-')[0], file.replace('json', 'png')) for file in os.listdir(self.output_dir + '/json/')]
        file_path_list = list(set(file_path_list) - set(processed_files))
        file_path_list.sort()
        print(f'{len(file_path_list)} files in queue')
