...
import os
import copy
from segmentation.misc.utils import log_info
from docopt import docopt

#-------------------------------------------------------------------------------------------------------

def run_infer(argv=None, counts=None):
    sub_cli_dict = {'tile' : tile_cli, 'wsi' : wsi_cli}
        
    args = docopt(__doc__, argv, help=False, options_first=True, 
    ...
    if sub_cmd == 'tile':
        from segmentation.infer.tile import InferManager
        infer = InferManager(**method_args)
        infer.process_file_list(run_args)
    else:
        from segmentation.infer.wsi import InferManager
        infer = InferManager(**method_args)
        infer.process_wsi_list(run_args, counts)
...
if __name__ == '__main__':
    argv = ['--gpu','1',
            ...
            '--batch_size','64',
            '--model_mode','original',
            '--model_path','segmentation/pretrained/hovernet_original_cpm17_notype_tf2pytorch.tar',
            '--nr_inference_workers','8',
            '--nr_post_proc_workers','8',
            'tile',
            '--input_dir',r'C:\Users\M306410\Downloads\TCGA\preprocessing\tiles_png',
            '--output_dir','results/segmentation/ihc2maskvar',
            '--mem_usage','0.1',
            # '--draw_dot',
            #'--save_qupath'
            ]
